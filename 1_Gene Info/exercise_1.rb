...
def process_file(file_path)
  ...
  File.foreach(file_path) do |line|
    ...
    if columns[0].to_s == "9606"
      amount_of_homo_sapiens += 1
    end
...
def calculate_MD5(file_path)
  return Digest::MD5.file(file_path).hexdigest
end
...
def find_highest_key(hash)
  "Most frequent gene type: #{hash.key(hash.values.max).to_s}, Amount of Times: #{hash.values.max.to_s}"
end
